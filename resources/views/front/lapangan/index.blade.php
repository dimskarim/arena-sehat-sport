...
<main class="pt-24 pb-20 max-w-7xl mx-auto px-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6 motion-hidden relative z-30">
        ...
        <div class="relative w-64 z-50 mt-4 md:mt-0">
            ...
            <div id="custom-dropdown-btn" class="flex items-center justify-between bg-white hover:bg-orange-50 transition-colors px-4 py-2.5 rounded-md border border-orange-500 cursor-pointer w-full z-50">
                <span class="text-gray-600 text-[15px]" id="custom-dropdown-text">
                    @php
                    $sortText = [
                    'terbaru' => 'Relevance',
            ...
            <div id="custom-dropdown-menu" class="absolute left-0 top-[110%] w-full bg-[#f9fafa] rounded-md shadow-[0_4px_15px_rgba(0,0,0,0.08)] hidden z-50">
                <!-- Upward Pointer -->
                <div class="absolute -top-1.5 left-8 w-3 h-3 bg-[#f9fafa] transform rotate-45 shadow-[-2px_-2px_4px_rgba(0,0,0,0.02)] border-t border-l border-gray-100"></div>

                <ul class="relative z-10 py-1" id="custom-dropdown-options">
                    <li data-value="terbaru" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('sort', 'terbaru') == 'terbaru' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Relevance</li>
                    <li data-value="nama_asc" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('sort') == 'nama_asc' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Product Name: A-Z</li>
                    <li data-value="nama_desc" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('sort') == 'nama_desc' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Product Name: Z-A</li>
                    <li data-value="harga_asc" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('sort') == 'harga_asc' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Price: Low to High</li>
                    <li data-value="harga_desc" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('sort') == 'harga_desc' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Price: High to Low</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <section class="mb-12 motion-hidden relative z-20">
        <form id="filter-form" action="{{ route('lapangan.index') }}" method="GET" class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl p-6 shadow-xl shadow-gray-200/50 dark:shadow-black/50 border border-white/50 dark:border-gray-700">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative w-full z-40">
                        ...
                        <!-- Custom Dropdown Button -->
                        <div id="custom-dropdown-kat-btn" class="flex items-center justify-between bg-white hover:bg-orange-50 transition-colors px-4 py-3.5 rounded-md border border-orange-500 cursor-pointer w-full z-40">
                            <span class="text-gray-600 text-[15px]" id="custom-dropdown-kat-text">
                                @php
                            ...
                            <svg id="custom-dropdown-kat-icon" class="w-4 h-4 text-gray-400 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        </div>
                        
                        <!-- Custom Dropdown Menu -->
                        <div id="custom-dropdown-kat-menu" class="absolute left-0 top-[110%] w-full bg-[#f9fafa] rounded-md shadow-[0_4px_15px_rgba(0,0,0,0.08)] hidden z-40">
                            <!-- Upward Pointer -->
                            <div class="absolute -top-1.5 left-8 w-3 h-3 bg-[#f9fafa] transform rotate-45 shadow-[-2px_-2px_4px_rgba(0,0,0,0.02)] border-t border-l border-gray-100"></div>
                            
                            <ul class="relative z-10 py-1" id="custom-dropdown-kat-options">
                                <li data-value="" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('kategori') == '' ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">Semua Olahraga</li>
                                @foreach($kategoris as $kat)
                                <li data-value="{{ $kat->id }}" class="px-4 py-2.5 text-[15px] cursor-pointer transition-colors {{ request('kategori') == $kat->id ? 'bg-[#f0f2f5] text-blue-600 font-medium' : 'text-gray-500 hover:bg-[#f0f2f5] hover:text-blue-600' }}">{{ $kat->name }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input name="search" value="{{ request('search') }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl pl-11 pr-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 text-gray-900 dark:text-white font-medium transition-all placeholder:text-gray-400" placeholder="Cari nama..." type="text" />
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input name="harga_max" value="{{ request('harga_max') }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl pl-11 pr-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 text-gray-900 dark:text-white font-medium transition-all placeholder:text-gray-400" placeholder="Rp 500.000" type="number" />
            ...
            <article class="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-lg shadow-gray-200/50 dark:shadow-black/20 border border-gray-100 dark:border-gray-800 group hover:shadow-2xl hover:shadow-red-900/10 hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full relative">
                ...
                <div class="p-6 flex flex-col flex-grow bg-white dark:bg-gray-900">
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-100 dark:border-gray-800">
                        <a href="{{ route('lapangan.show', $lapangan->id) }}" class="w-full flex items-center justify-center gap-2 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-900/30 px-6 py-3 rounded-xl font-bold transition-all hover:bg-red-600 hover:text-white dark:hover:bg-red-600 dark:hover:text-white group-hover:shadow-lg group-hover:shadow-red-600/20">
                            Booking
