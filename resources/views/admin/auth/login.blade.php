...
<body class="bg-background text-on-background min-h-screen flex flex-col items-center justify-center relative overflow-x-hidden">
    ...
    <main class="w-full max-w-md px-6 py-12 md:py-24">
        ...
        <div class="text-center mb-10 animate-fade-up">
            ...
            <h1 class="text-3xl font-bold tracking-tight text-on-surface mb-2 font-headline">Portal Admin & Pemilik</h1>
            <p class="text-on-surface-variant font-body">Masuk ke dashboard untuk mengelola fasilitas Anda.</p>
        </div>
        ...
        <!-- Form Fields -->
        <form class="space-y-5 animate-fade-up delay-300" action="{{ route('admin.login.submit') }}" method="POST">
            @csrf
            @if ($errors->any())
        ...
        <p class="text-center mt-8 text-sm text-on-surface-variant font-body animate-fade-up delay-400">
            Belum punya akun pemilik? <a class="text-primary font-bold hover:underline transition-all" href="{{ route('admin.register') }}">Daftar sekarang</a>
        </p>
