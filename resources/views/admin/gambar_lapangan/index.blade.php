...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <h2 class="text-title-md2 font-bold text-black dark:text-white">
        Data Gambar Lapangan
    </h2>

    <nav>
        <a href="{{ route('admin.gambar-lapangans.create') }}" class="inline-flex items-center justify-center rounded-md bg-brand-500 py-2 px-6 text-center font-medium text-white hover:bg-opacity-90">
            Tambah Data
        </a>
    </nav>
</div>
        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark"><p class="text-black dark:text-white">{{ $item->gambar ?? '-' }}</p></td>

                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div class="flex items-center space-x-3.5">
                            <a href="{{ route('admin.gambar-lapangans.edit', $item->id) }}" class="hover:text-brand-500">Edit</a>
                            <form action="{{ route('admin.gambar-lapangans.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus?');">
                                @csrf
                                @method('DELETE')
