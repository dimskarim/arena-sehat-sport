    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl border border-[#e4beba]/50 shadow-sm overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                ...
                <form id="kategoriEditForm" action="{{ route('admin.kategoris.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
            ...
            <div class="bg-white rounded-xl border border-[#e4beba]/50 shadow-sm overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                ...
                <div class="p-6 flex flex-col items-center text-center">
                    <div class="w-16 h-16 rounded-2xl bg-[#fdcbd0] flex items-center justify-center mb-4 border border-[#e4beba] dark:border-gray-700 overflow-hidden">
                        <img id="previewLogo" src="{{ $item->logo ? asset('storage/' . $item->logo) : '' }}" alt="Logo" class="w-full h-full object-cover {{ $item->logo ? '' : 'hidden' }}">
                        <svg id="defaultLogoIcon" class="text-[#af101a] text-3xl inline-block align-middle w-5 h-5 {{ $item->logo ? 'hidden' : '' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
...
<script>
    ...
    const hasOriginalLogo = {{ $item->logo ? 'true' : 'false' }};
    const originalLogoSrc = "{{ $item->logo ? asset('storage/' . $item->logo) : '' }}";
    ...
    async function executeDelete() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('admin.kategoris.index') }}";
                }, 1000);
            } else {
