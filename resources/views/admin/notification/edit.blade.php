    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-gray-400 mb-2">
                ...
                <span class="font-semibold text-slate-900 dark:text-white">Edit Notifikasi</span>
            </div>
            <h1 class="font-['Lexend'] text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Notifikasi</h1>
            <p class="text-slate-500 dark:text-gray-400 mt-1 text-sm">Perbarui isi pesan atau detail notifikasi ini.</p>
        </div>
        <button type="submit" form="editNotifForm" class="flex items-center gap-2 px-6 py-2.5 bg-red-700 dark:bg-red-600 text-white text-sm font-semibold rounded-xl hover:opacity-90 shadow-lg shadow-red-900/20 active:scale-95 transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <form id="editNotifForm" action="{{ route('admin.notifications.update', $item->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="pesan" required rows="5" placeholder="Tulis pesan notifikasi di sini..."
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-red-100 focus:border-red-600 dark:border-red-500 outline-none transition-all resize-none">{{ old('pesan', $item->pesan) }}</textarea>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 dark:border-gray-700 shadow-sm p-6">
                <h4 class="font-['Lexend'] font-bold text-slate-900 dark:text-white mb-4">Detail Notifikasi</h4>
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        <span class="text-slate-500 dark:text-gray-400">ID Notifikasi</span>
                        <span class="font-bold text-slate-900 dark:text-white">#{{ $item->id }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500 dark:text-gray-400">Status Baca</span>
                        <span class="font-bold {{ $item->is_read ? 'text-green-600' : 'text-amber-600' }}">{{ $item->is_read ? 'Sudah Dibaca' : 'Belum Dibaca' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500 dark:text-gray-400">Dibuat</span>
                        <span class="font-medium text-slate-900 dark:text-white">{{ $item->created_at ? $item->created_at->format('d M Y') : '-' }}</span>
                    </div>
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 dark:border-gray-700 shadow-sm p-6">
                <h4 class="font-['Lexend'] font-bold text-slate-900 dark:text-white mb-4">Hapus Notifikasi</h4>
                <p class="text-sm text-slate-500 dark:text-gray-400 mb-4">Tindakan ini tidak bisa dibatalkan.</p>
                <form action="{{ route('admin.notifications.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus notifikasi ini?');">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full py-2.5 rounded-xl bg-red-50 text-red-600 dark:text-red-400 border border-red-200 text-sm font-bold hover:bg-red-100 transition-colors">
